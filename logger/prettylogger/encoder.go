...
func NewEncoder(cfg zapcore.EncoderConfig) zapcore.Encoder {
	return &recordingEncoder{e: prettyloggerEncoder{
		buf:             nil,
		cfg:             &cfg,
		level:           0,
		namespaceIndent: 0,
...
var _ zapcore.Encoder = (*prettyloggerEncoder)(nil)

type prettyloggerEncoder struct {
	buf *buffer.Buffer
...
func (e prettyloggerEncoder) clone() *prettyloggerEncoder {
	clone := getPrettyloggerEncoder()
	clone.buf = getBuffer()
...
func (e prettyloggerEncoder) EncodeEntry(entry zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	e.buf = getBuffer()
	e.level = entry.Level
...
func (e *prettyloggerEncoder) addSeparator() {
	if e.inList {
...
// []byte.
func (e *prettyloggerEncoder) appendSafeByte(s []byte) {
	for i := 0; i < len(s); {
		if e.tryAddRune(s[i]) {
			i++
			continue
...
// single byte.
func (e *prettyloggerEncoder) tryAddRune(b byte) bool {
...
func (e *prettyloggerEncoder) tryAddRuneError(r rune, size int) bool {
